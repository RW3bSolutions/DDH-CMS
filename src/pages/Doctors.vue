    ...
    <div v-else>
      <div class="row q-px-sm">
        ...
        <div class="col-4 col-md-2 q-py-sm self-center text-right">
          <q-btn icon="add" label="New Doctor" class="full-width" color="green" size="md" @click="create()" />
        </div>
      </div>
  ...
  <q-dialog v-model="modal">
    <q-card style="width: 700px; max-width: 80vw;">
      <form @submit.prevent="ifEdit ? update() : store()">
        <q-card-section>
          <div class="text-h6">{{ ifEdit ? 'UPDATE DOCTOR' : 'NEW DOCTOR' }}</div>
...
export default defineComponent({
  ...
  methods: {
    ...
    update () {
      this.submitted = true
      this.errors.show = false
      this.$api.post('/doctors/' + this.object.id, {
        name: this.form.name,
        role: this.form.role,
        username: this.form.username,
        is_active: this.form.is_active,
